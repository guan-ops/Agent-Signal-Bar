// Adapted from guan-ops/CodexBar, commit 1696c7a. MIT; see Resources/ClaudeSupport-LICENSE.txt.
...
public enum ClaudeSwapListParser {
    ...
    public static func parse(_ data: Data) throws -> ClaudeSwapAccountList {
        ...
        case is NSNull: nil
        case let raw where integer(raw).map({ $0 > 0 }) == true: integer(raw)
        default:
    ...
    private static func parseRow(_ row: [String: Any]) throws -> ClaudeSwapAccountRow {
        guard let number = integer(row["number"]) else {
